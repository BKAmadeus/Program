function ST01()
    ...
    x1 = round(2233/3.9313);
    x2 = round(1993/3.9313);
    y1 = round(920.3/3.9498);
    y2 = round(892.7/3.9498);
    
    x1 = 377;
    x2 = 433;
    y1 = 158;
    y2 = 214;
    [MaxElevation,test] = StraightPosition(xpixel,ypixel,frequency,x1,y1,gioData(y1,x1)+0.2,x2,y2,gioData,0.5,gioMeta);
    THR = ThresholdGain(xpixel,ypixel,threshold,frequency,x1,y1,gioData(y1,x1)+0.2,x2,y2,gioData(y2,x2)+0.5,MaxElevation);
   ...
   prompt = 'contine != 1 ';
    ...
    while flg == 1
        ...
        y2 = input(prompt);
        hold on
        plot3([x1 x2],[y1 y2],[gioData(y1,x1) gioData(y2,x2)],'-black', 'LineWidth',2);
        hold off
        d =  getDistance3D(x1*xpixel,y1*ypixel,gioData(y1,x1),x2*xpixel,y2*ypixel,gioData(y2,x2));
        do =  getDistance3D(x1*xpixel,y1*ypixel,gioData(y1,x1),x2*xpixel,y2*ypixel,gioData(y1,x1));
        de =  getDistance3D(x2*xpixel,y2*ypixel,gioData(y1,x1),x2*xpixel,y2*ypixel,gioData(y2,x2));
        disp(d);
        disp(do);
        disp(de);
        %disp(do / 100);
    ...
    %plot(test(:,1),test(:,9));
    %plot(test(:,2),test(:,6));
    %scatter(test(:,2),test(:,7));
    %scatter(test(:,2),test(:,8));
    %plot(test(:,2),test(:,9));
    %scatter(MaxElevation(1),MaxElevation(3));
    %scatter(MaxElevation(1),MaxElevation(3));
    %scatter(MaxElevation(8),MaxElevation(10));
    %hold off
    %{
    hold on
    plot3([x1*xpixel x2*xpixel],[y1*ypixel y2*ypixel],[gioData(y1,x1) gioData(y2,x2)],'-black', 'LineWidth',2);
    hold off
...
function h = NewNodeHeight(xpixel,ypixel,WaveMap,nodeList,ConnectMini,gioData,xyz,minh,maxh,interval,frequency,threshold,AddPoint)
    ...
    if numel(A) ~= 1
        for i = maxh:-interval:minh
            for j = 1:length(A)
                MaxElevation = StraightPosition(xpixel,ypixel,frequency,xyz(A(j),1),xyz(A(j),2),xyz(A(j),3),AddPoint(1),AddPoint(2),gioData,i);
        ...
        for i = maxh:-interval:minh
            for j = 1:length(C)
                MaxElevation = StraightPosition(xpixel,ypixel,frequency,xyz(C(j),1),xyz(C(j),2),xyz(C(j),3),AddPoint(1),AddPoint(2),gioData,i);
...
function AddPoint = FarDeploy(xpixel,ypixel,MAPS,gioData,WaveMap,ConnectMini,nodeList,height,frequency,xyz)
    AddPoint = [0 0 0];
    [m,n] = size(gioData);
    MAP = zeros(m,n);
    dMax = -10000;
    ...
    for j = 1:Le
        for i = 1:length(B)
            ...
            x0 = ecol(j);
            y0 = erow(j);
            MaxElevation = StraightPosition(xpixel,ypixel,frequency,x,y,z,x0,y0,gioData,height(y0,x0));
        ...
        for y = 1:m
            for x = 1:n
                if MtMax == Aggregation(y,x)
                    ...
                    for i = 1:length(D)
                        for j = 1:length(Df)
                            MaxElevation = StraightPosition(xpixel,ypixel,frequency,xyz(i,1),xyz(i,2),xyz(i,3),xyz(j,1),xyz(j,2),Matrix,height(xyz(j,2),xyz(j,1)));
                            f = ThresholdGain(xpixel,ypixel,Max,frequency,x,y,Matrix(y,x)+height(y,x),xyz(i,1),xyz(i,2),xyz(i,3),MaxElevation);
                            if f > Max
                                fMax = f;
                            end
                        end
                        MaxElevation = StraightPosition(xpixel,ypixel,frequency,xyz(i,1),xyz(i,2),xyz(i,3),x,y,Matrix,height(y,x));
                        f = ThresholdGain(xpixel,ypixel,Max,frequency,x,y,Matrix(y,x)+height(y,x),xyz(i,1),xyz(i,2),xyz(i,3),MaxElevation);
                        dif = f - fMax;
                        if dif > diffMax
                            Point(1) = x;
...
function PointThreshold = ElectromagneticWaveThreshold(xpixel,ypixel,threshold,frequency,gioData,x,y,z,xyse,height)
    [m,n] = size(gioData);
    PointThreshold = zeros(m,n);
    for i = xyse(1,1) : xyse(1,2)
        for j = xyse(2,1) : xyse(2,2)
            MaxElevation = StraightPosition(xpixel,ypixel,frequency,x,y,z,i,j,gioData,height(j,i));
            PointThreshold(j,i) = ThresholdGain(xpixel,ypixel,threshold,frequency,x,y,z,i,j,gioData(j,i)+height(j,i),MaxElevation);
        end
    end
end

function f = ThresholdGain(xpixel,ypixel,threshold,frequency,x,y,z,x1,y1,z1,MaxElevation)
    %LoS環境
    ...
    %
    if MaxElevation(6) <= 0
        f = f - 3*MaxElevation(6);
    end
end

function [MaxElevation,test] = StraightPosition(xpixel,ypixel,frequency,TyuX,TyuY,TyuZ,X,Y,gioData,h,gioMeta)
    ...
    if X == TyuX  || abs(Tilt) >= 1
        ...
        MaxElevation =  [maxX maxY maxZ 0 0 0 0 minX minY minZ 0 0];
        sita = 180 / (maxY - minY);
        for i = minY : maxY
            if X == TyuX
                x = TyuX;
            ...
            %arad = getRadian2D(maxY, maxZ, y,  z);
            %brad = getRadian2D(minY, minZ, y,  z);
            FrRange = TiltZ - Fr;
            ...
            %{
                ...
                disp("test01");
                disp(arad - getRadian2D(maxY, maxZ, y,  TiltZ));
                disp("test02");
                disp(arad);
                if srad < arad - getRadian2D(maxY, maxZ, y,  TiltZ)
                    MaxElevation(11) = arad;
                    srad = arad - getRadian2D(maxY, maxZ, y,  TiltZ);
                end
                if erad < brad - getRadian2D(minY, minZ, y,  TiltZ)
                    MaxElevation(8) = x;
                    MaxElevation(9) = y;
                    MaxElevation(10) = z;
                    MaxElevation(12) = brad;
                    erad = brad - getRadian2D(minY, minZ, y,  TiltZ);
                end
            %}
    ...
    elseif Y == TyuY || abs(Tilt) < 1
        ...
        for i = minX : maxX
            ...
            arad = getRadian2D(maxX, maxZ, x,  z);
            brad = getRadian2D(minX, minZ, x,  z);
            Fr = sin(Sin) * FreR;
            FrRange = TiltZ - Fr;
            Stem = (z + 3)*(gioMeta(y,x) == 1000);
            Forest = Stem + 7;
