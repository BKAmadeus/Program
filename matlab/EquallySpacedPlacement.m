function EquallySpacedPlacement()
     clear
    ...
    for i = 1:numel(nodeList)
        ...
        PointThreshold = ElectromagneticWaveThreshold(xpixel,ypixel,threshold,frequency,gioData,xyz(i,1),xyz(i,2),xyz(i,3),xyse,height);
        [WaveMap,MAPS,maps] = ElectromagneticWaveMap(PointThreshold,WaveMap,i,xyse,MAPS,maps,gioData);
    end
    ...
    while numel(ConnectMini) ~= numel(find(ConnectMini == 1))
        [AddPoint,MtMax] = OverLap(xpixel,ypixel,gioData,nodeList,MAPS(:,:,2),WaveMap,ConnectMini,height,frequency,xyz);
        if MtMax == 0.25
             [AddPoint] = FarDeploy(xpixel,ypixel,MAPS,gioData,WaveMap,ConnectMini,nodeList,height,frequency,xyz);
        end
        h = NewNodeHeight(xpixel,ypixel,WaveMap,nodeList,ConnectMini,gioData,xyz,minh,maxh,interval,frequency,threshold,AddPoint);
        [xyz,nodeNum,nodeList] = AddnodeList(nodeList,AddPoint,xyz,lat,lon,h);
        xyse = LoopStartEnd(AddPoint,xRSSI,yRSSI,m,n);
        AddPointThreshold = ElectromagneticWaveThreshold(xpixel,ypixel,threshold,frequency,gioData,xyz(nodeNum,1),xyz(nodeNum,2),xyz(nodeNum,3),xyse,height);
        [WaveMap,MAPS,maps] = ElectromagneticWaveMap(AddPointThreshold,WaveMap,nodeNum,xyse,MAPS,maps,gioData);
        nodeList = DrawLine(xyz,WaveMap,nodeList,xpixel,ypixel);
        ConnectMini = ConnectMinimum(nodeList);
    end
...
function h = NewNodeHeight(xpixel,ypixel,WaveMap,nodeList,ConnectMini,gioData,xyz,minh,maxh,interval,frequency,threshold,AddPoint)
    ...
    if numel(A) ~= 1
        for i = maxh:-interval:minh
            for j = 1:length(A)
                MaxElevation = StraightPosition(xyz(A(j),1),xyz(A(j),2),xyz(A(j),3),AddPoint(1),AddPoint(2),gioData,i);
        ...
        for i = maxh:-interval:minh
            for j = 1:length(C)
                MaxElevation = StraightPosition(xyz(C(j),1),xyz(C(j),2),xyz(C(j),3),AddPoint(1),AddPoint(2),gioData,i);
...
function AddPoint = FarDeploy(xpixel,ypixel,MAPS,gioData,WaveMap,ConnectMini,nodeList,height,frequency,xyz)
    ...
    for j = 1:Le
        for i = 1:length(B)
            ...
            x0 = ecol(j);
            y0 = erow(j);
            MaxElevation = StraightPosition(x,y,z,x0,y0,gioData,height(y0,x0));
        ...
        for y = 1:m
            for x = 1:n
                if MtMax == Aggregation(y,x)
                    ...
                    for i = 1:length(D)
                        for j = 1:length(Df)
                            MaxElevation = StraightPosition(xyz(i,1),xyz(i,2),xyz(i,3),xyz(j,1),xyz(j,2),Matrix,height(xyz(j,2),xyz(j,1)));
                            f = ThresholdGain(xpixel,ypixel,Max,frequency,x,y,Matrix(y,x)+height(y,x),xyz(i,1),xyz(i,2),xyz(i,3),MaxElevation);
                            if f > Max
                                fMax = f;
                            end
                        end
                        MaxElevation = StraightPosition(xyz(i,1),xyz(i,2),xyz(i,3),x,y,Matrix,height(y,x));
                        f = ThresholdGain(xpixel,ypixel,Max,frequency,x,y,Matrix(y,x)+height(y,x),xyz(i,1),xyz(i,2),xyz(i,3),MaxElevation);
                        dif = f - fMax;
                        if dif > diffMax
                            Point(1) = x;
...
function DrawPoint(gioData, nodeList,xyz,xpixel,ypixel,maps,MAPS)
    ...
    for i = 1:nodeNum
        ...
        if (strcmp(nodeList(i).type, 'ed'))
            scatter3(xyz(i,1)*xpixel, xyz(i,2)*ypixel, xyz(i,3)+10, 75,'white','filled','o','LineWidth',3,'MarkerEdgeColor','k');
        % 端末がエンドデバイス
        elseif (strcmp(nodeList(i).type, 'rt'))
            scatter3(xyz(i,1)*xpixel, xyz(i,2)*ypixel, xyz(i,3)+10, 75 ,'white','filled','^','LineWidth',2,'MarkerEdgeColor','k'); %[R,G,B]
        % 端末がエンドデバイス
        elseif (strcmp(nodeList(i).type, 'cd'))
            scatter3(xyz(i,1)*xpixel, xyz(i,2)*ypixel, xyz(i,3)+10, 300,'white','filled','p','LineWidth',2,'MarkerEdgeColor','k');
        end
        % 端末情報の描画
...
function MaxElevation = StraightPosition(TyuX,TyuY,TyuZ,X,Y,gioData,h)
    %map(y,x) maxelevation(x y z z0) 
    MaxElevation =  [TyuX TyuY TyuZ 0];
    flg = 0;
    Tilt = (TyuY - Y)/(TyuX - X);
    ...
    if flg >= 1
        ...
        StandardTiltZ = (gioData(Y,X) + h - TyuZ)/(Y - TyuY);
        StandardSectionZ = StandardTiltZ * -TyuY + TyuZ;
        for i = minY : maxY
            if flg == 1
    ...
    elseif flg == 0
        ...
        StandardTiltZ = (gioData(Y,X) + h - TyuZ)/(X - TyuX);
        StandardSectionZ = StandardTiltZ * -TyuX + TyuZ;
        for i = minX : maxX
            x = i;
