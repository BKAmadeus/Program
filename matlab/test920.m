...
function [xRSSI,yRSSI] = FreeRSSICreat(threshold,frequency,xpixel,ypixel)
    d = sqrt((0.08*0.08*(300 / frequency)^2 * 10.^(-threshold / 10)) / (4 * pi)^2);
...
function f = ThresholdGain(xpixel,ypixel,frequency,x,y,z,x1,y1,z1,MaxElevation)
    ...
    %
    if MaxElevation(6) >= 0
        f = f - sqrt(0.1*MaxElevation(6));
    end
    if MaxElevation(7) >= 0
        f = f - sqrt(1.0*MaxElevation(7));
    end
    if MaxElevation(8) >= 0
        f = f - sqrt(0.8*MaxElevation(8));
    end
end
