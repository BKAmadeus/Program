...
function h = NewNodeHeight(xpixel,ypixel,WaveMap,nodeList,gioData,xyz,minh,maxh,interval,frequency,threshold,AddPoint)
    ...
    for i = maxh:-interval:minh
        ...
        [MaxElevation,XElevation,YElevation,ZElevation,FElevation] = StraightPosition(xpixel,ypixel,frequency,xyz(A,1),xyz(A,2),xyz(A,3),At,gioData,0);
        Threshold = ThresholdGain(xpixel,ypixel,frequency,At,xyz(A,3),MaxElevation,xyz(A,1),xyz(A,2),XElevation,YElevation,ZElevation,FElevation);
...
function Point = FarDeploy(xpixel,ypixel,MAPS,gioData,WaveMap,ConnectMini,nodeList,height,frequency,xyz)
    ...
    Data = gioData(CI);
    hi = height(CI);
    [MaxElevation,XElevation,YElevation,ZElevation,FElevation] = StraightPosition(xpixel,ypixel,frequency,XI,YI,Data,xyz(A(1),:),gioData,hi);
    PointThreshold = ThresholdGain(xpixel,ypixel,frequency,xyz(A(1),:),Data+hi,MaxElevation,XI,YI,XElevation,YElevation,ZElevation,FElevation);
...
function [Point,MtMax] = OverLap(xpixel,ypixel,Matrix,nodeList,Aggregation,WaveMap,ConnectMini,height,frequency,xyz)
    ...
    if MtMax ~= 0.25
        ...
        for i = 1:nMax
            ...
            t = size(CIf);
            if t(1) ~= 0
                [MaxElevation,XElevation,YElevation,ZElevation,FElevation] = StraightPosition(xpixel,ypixel,frequency,xyz(:,1),xyz(:,2),xyz(:,3),xyz(nMax - i + 1,:),Matrix,0);
                PointThreshold = ThresholdGain(xpixel,ypixel,frequency,xyz(nMax - i + 1,:),xyz(:,3),MaxElevation,xyz(:,1),xyz(:,2),XElevation,YElevation,ZElevation,FElevation);
                nodeThreshold = maxk(PointThreshold,2);
                XE = X(CIf);
                YE = Y(CIf);
                M = Data(CIf);
                hig = hi(CIf);
                [MaxElevation,XElevation,YElevation,ZElevation,FElevation] = StraightPosition(xpixel,ypixel,frequency,XE,YE,M,xyz(nMax - i + 1,:),Matrix,hig);
                PointThreshold = ThresholdGain(xpixel,ypixel,frequency,xyz(nMax - i + 1,:),M+hig,MaxElevation,XE,YE,XElevation,YElevation,ZElevation,FElevation);
                PointThreshold = PointThreshold - nodeThreshold(2);
                [fMax,MaxIndex] = max(PointThreshold);
            end
            if t(1) == 0
                [MaxElevation,XElevation,YElevation,ZElevation,FElevation] = StraightPosition(xpixel,ypixel,frequency,X,Y,Data,xyz(nMax - i + 1,:),Matrix,hi);
                PointThreshold = ThresholdGain(xpixel,ypixel,frequency,xyz(nMax - i + 1,:),Data+hi,MaxElevation,X,Y,XElevation,YElevation,ZElevation,FElevation);
                [fMax,MaxIndex] = max(PointThreshold);
            end
            if diffMax < (fMax)
                Point(1) = X(MaxIndex);
...
function DrawPoint(gioData, nodeList,xyz,xpixel,ypixel,maps,MAPS)
    ...
    for i = 1:nodeNum
        ...
        elseif (strcmp(nodeList(i).type, 'rt'))
            scatter3(xyz(i,1)*xpixel, xyz(i,2)*ypixel, xyz(i,3)+10, 300 ,'red','filled','^','LineWidth',2,'MarkerEdgeColor','k'); %[R,G,B]
        % 端末がエンドデバイス
        elseif (strcmp(nodeList(i).type, 'cd'))
            scatter3(xyz(i,1)*xpixel, xyz(i,2)*ypixel, xyz(i,3)+10, 600,'yellow','filled','p','LineWidth',2,'MarkerEdgeColor','k');
        end
        % 端末情報の描画
         text(xyz(i,1)*xpixel+10, xyz(i,2)*ypixel , xyz(i,3) + 30, num2str(nodeList(i).no) ,'Color','black','FontSize',15,'FontWeight','bold');
    end
...
function PointThreshold =   ElectromagneticWaveThreshold(xpixel,ypixel,frequency,gioData,xyz,xyse,height)
    ...
    [MaxElevation,XElevation,YElevation,ZElevation,FElevation] = StraightPosition(xpixel,ypixel,frequency,X,Y,Data,xyz,gioData,high);%FreR
    PointThreshold = ThresholdGain(xpixel,ypixel,frequency,xyz,Data+high,MaxElevation,X,Y,XElevation,YElevation,ZElevation,FElevation);
end

function Threshold = ThresholdGain(xpixel,ypixel,frequency,xyz,gioData,MaxElevation,X,Y,XElevation,YElevation,ZElevation,FElevation)
    d = getDistance3D(xyz(1)*xpixel,xyz(2)*ypixel,xyz(3),X*xpixel,Y*ypixel,gioData);
    f = calFreeSpaceGain(d,frequency);
    Threshold = f;
    
    %FRENEL = (FElevation < 1 & FElevation > 0);
    %Threshold(FRENEL) = Threshold(FRENEL) - 6*FElevation(FRENEL);
    
    KNIFEEDGE = (MaxElevation > 0);
    ...
    Threshold(KNIFEEDGE) = Threshold(KNIFEEDGE) - v;
    
end

function [MaxElevation,XElevation,YElevation,ZElevation,FElevation] = StraightPosition(xpixel,ypixel,f,X,Y,Data,xyz,gioData,high)%FreR
    MinY = Y;
    MaxY = Y;
    MinX = X;
    MaxX = X;
    ...
    d = getDistance3D(xyz(1)*xpixel,xyz(2)*ypixel,xyz(3),X*xpixel,Y*ypixel,Data);
    FreR = sqrt(300/f*d)/2;
    sita = 180 ./ AT;
    ...
    for i = s:e
        ...
        FrMax = ST + Fr;
        FrMin = ST - Fr;
        FrRange = (Z - FrMin)./(FrMax - FrMin);
        RZ = Z - ST;
        FrChack = (FElevation < FrRange);
        FElevation(FrChack) = FrRange(FrChack);
