function test2400()
    ...
    for i = 1:numel(nodeList)
        ...
        PointThreshold = ElectromagneticWaveThreshold(xpixel,ypixel,frequency,gioData,xyz(i,1),xyz(i,2),xyz(i,3),xyse,height,gioMeta);
        [WaveMap,MAPS,maps] = ElectromagneticWaveMap(PointThreshold,threshold,WaveMap,i,xyse,MAPS,maps,gioData);
    end
    nodeList = DrawLine(xyz,WaveMap,nodeList,xpixel,ypixel);
    makeOutputXML(nodeList,commonInfo,'Middle\node.xml');
    dlmwrite('Middle\Aggregation.csv',MAPS(:,:,3));
    dlmwrite('Middle\map.csv',gioData);
    ConnectMini = ConnectMinimum(nodeList);
    %pause;
    %ノード追加を行い配列に格納する
    while numel(ConnectMini) ~= numel(find(ConnectMini == 1))
        [AddPoint,MtMax] = OverLap(xpixel,ypixel,gioData,nodeList,MAPS(:,:,2),WaveMap,ConnectMini,height,frequency,xyz,gioMeta);
        if MtMax == 0.25
             [AddPoint] = FarDeploy(xpixel,ypixel,MAPS,gioData,WaveMap,ConnectMini,nodeList,height,frequency,xyz,gioMeta);
        end
        h = maxh;
        %h = NewNodeHeight(xpixel,ypixel,WaveMap,nodeList,ConnectMini,gioData,xyz,minh,maxh,interval,frequency,threshold,AddPoint,gioMeta);
        height(AddPoint(2),AddPoint(1)) = h;
        [xyz,nodeNum,nodeList] = AddnodeList(nodeList,AddPoint,xyz,lat,lon,h);
        xyse = LoopStartEnd(xyz(end,:),xRSSI,yRSSI,m,n);
        AddPointThreshold = ElectromagneticWaveThreshold(xpixel,ypixel,frequency,gioData,xyz(end,1),xyz(end,2),xyz(end,3),xyse,height,gioMeta);
        if nodeNum == 30
            dlmwrite('result\test.csv',AddPointThreshold);
        end
        [WaveMap,MAPS,maps] = ElectromagneticWaveMap(AddPointThreshold,threshold,WaveMap,nodeNum,xyse,MAPS,maps,gioData);
        nodeList = DrawLine(xyz,WaveMap,nodeList,xpixel,ypixel);
        ConnectMini = ConnectMinimum(nodeList);
    end
    dlmwrite('result\result_Aggregation.csv',MAPS(:,:,2));
    
    %savefig("untitled.fig");
    makeOutputXML(nodeList,commonInfo,'result\result_node.xml')
    toc
    pro = 'exit = 0 or contine = 1 ';
    %flg = input(pro);
    flg = 0;
    while flg == 1
        prompt = 'What is the original X - value? ';
        x = input(prompt);
        prompt = 'What is the original Y - value? ';
        y = input(prompt);
        h = maxh;
        AddPoint = [x y gioData(y,x)+h];
        [xyz,nodeNum,nodeList] = AddnodeListF(nodeList,AddPoint,xyz,lat,lon,h);
        xyse = LoopStartEnd(xyz(end,:),xRSSI,yRSSI,m,n);
        AddPointThreshold = ElectromagneticWaveThreshold(xpixel,ypixel,frequency,gioData,xyz(end,1),xyz(end,2),xyz(end,3),xyse,height,gioMeta);
        [WaveMap,MAPS,maps] = ElectromagneticWaveMap(AddPointThreshold,threshold,WaveMap,nodeNum,xyse,MAPS,maps,gioData);
        nodeList = DrawLine(xyz,WaveMap,nodeList,xpixel,ypixel);
        ConnectMini = ConnectMinimum(nodeList);
        %DrawPoint(gioData, nodeList,xyz,xpixel,ypixel,maps,MAPS,gioMeta);
        flg = input(pro);
    end
    DrawPoint(gioData, nodeList,xyz,xpixel,ypixel,maps,MAPS,gioMeta);
end
    ...
    if numel(A) ~= 1
        for i = maxh:-interval:minh
            for j = 1:length(A)
                ...
                Threshold = ThresholdGain(xpixel,ypixel,frequency,xyz(A(j),1),xyz(A(j),2),xyz(A(j),3),AddPoint(1),AddPoint(2),gioData(AddPoint(2),AddPoint(1))+i,MaxElevation);
        ...
        for i = maxh:-interval:minh
            for j = 1:length(C)
                ...
                Threshold = ThresholdGain(xpixel,ypixel,frequency,xyz(C(j),1),xyz(C(j),2),xyz(C(j),3),AddPoint(1),AddPoint(2),gioData(AddPoint(2),AddPoint(1))+i,MaxElevation);
                if Threshold >= max
                    h = i;
                end
            end
...
function AddPoint = FarDeploy(xpixel,ypixel,MAPS,gioData,WaveMap,ConnectMini,nodeList,height,frequency,xyz,gioMeta)
    ...
    for j = 1:Le
        for i = 1:length(B)
            ...
            f = ThresholdGain(xpixel,ypixel,frequency,x,y,z,x0,y0,gioData(y0,x0)+height(y0,x0),MaxElevation);
...
function [xyz,nodeNum,nodeList] = AddnodeListF(nodeList,AddPoint,xyz,lat,lon,h)
    latitude = lat(AddPoint(2));
    longitude = lon(AddPoint(1));
    nodeNum = numel(nodeList) + 1;
    nodeList(nodeNum).no        = nodeNum;
    nodeList(nodeNum).type      = 'rf';
    nodeList(nodeNum).lon       = longitude;
    nodeList(nodeNum).lat       = latitude;
    nodeList(nodeNum).height    = h;
    nodeList(nodeNum).connected = NaN;
                ...
                if MtMax == Aggregation(y,x)
                    ...
                    for i = 1:length(D)
                        for j = 1:length(Df)
                            ...
                            f = ThresholdGain(xpixel,ypixel,frequency,x,y,Matrix(y,x)+height(y,x),xyz(i,1),xyz(i,2),xyz(i,3),MaxElevation);
                            if f > Max
                                fMax = f;
                            end
                        end
...
function nodeList = DrawLine(xyz,WaveMap,nodeList,xpixel,ypixel)
    for i = 1:numel(nodeList)
        ...
        for j = A
            ...
            count = count + 1;
            hold on
             if (strcmp(nodeList(i).type, 'rf')) || (strcmp(nodeList(j).type, 'rf'))
                plot3([xyz(i,1)*xpixel xyz(j,1)*xpixel],[xyz(i,2)*ypixel xyz(j,2)*ypixel],[xyz(i,3) xyz(j,3)],':black', 'LineWidth',4);
            else
                plot3([xyz(i,1)*xpixel xyz(j,1)*xpixel],[xyz(i,2)*ypixel xyz(j,2)*ypixel],[xyz(i,3) xyz(j,3)],'-black', 'LineWidth',4);
            end
            %plot3([xyz(i,1) xyz(j,1)],[xyz(i,2) xyz(j,2)],[xyz(i,3) xyz(j,3)],'-black', 'LineWidth',2);
            %plot3([xyz(i,1)*xpixel xyz(j,1)*xpixel],[xyz(i,2)*ypixel xyz(j,2)*ypixel],[xyz(i,3) xyz(j,3)],'-black', 'LineWidth',4);
            hold off
        end
...
function [xRSSI,yRSSI] = FreeRSSICreat(threshold,frequency,xpixel,ypixel)
    d = sqrt((0.21*0.21*(300 / frequency)^2 * 10.^(-threshold / 10)) / (4 * pi)^2);
...
function DrawPoint(gioData,nodeList,xyz,xpixel,ypixel,maps,MAPS,gioMeta)
    ...
    X = imresize(X,2^3);
    Y = imresize(Y,2^3);
    gioData = imresize(gioData,2^3);
    MAPS = imresize(MAPS,2^3,'nearest');
    maps = imresize(maps,2^3,'nearest');
    gioMeta = imresize(gioMeta,2^3,'nearest');
    [I,map] = imread('Practice/map_18.jpg');
    [m1,n1,o] = size(I);
    X = X(1:m1,1:n1);
    Y = Y(1:m1,1:n1);
    gioMeta = gioMeta(1:m1,1:n1);
    gioData = gioData(1:m1,1:n1);
    MAPS = MAPS(1:m1,1:n1,:);
    maps = maps(1:m1,1:n1,:);
    
    %[M,c] = contour3(X,Y,gioData,35,'black');
    [M,c] = contour3(X*xpixel,Y*ypixel,gioData,35);
    c.LineWidth = 3;
    ...
    %s = surf(X,Y,maps,MAPS,'LineStyle' ,'none');
    %alpha(s,.5);
    ...
    s = warp(X*xpixel,Y*ypixel,gioData,I);
    alpha(s,.7);
    %s = warp(X,Y,gioData,I2);
    
    %s = surf(X*xpixel,Y*ypixel,maps,MAPS,'LineStyle' ,'none');
    %alpha(s,.5);
    %s = surf(X*xpixel,Y*ypixel,gioData,MAPS,'LineStyle' ,'none');
    %alpha(s,.3);
    %MAPS(:,:,2) = 1 - MAPS(:,:,2);
    %MAPS(:,:,3) = MAPS(:,:,2);
    %s = surf(X*xpixel,Y*ypixel,maps,MAPS,'LineStyle' ,'none');
    ...
    for i = 1:nodeNum
        ...
        if (strcmp(nodeList(i).type, 'ed'))
            %scatter3(xyz(i,1), xyz(i,2), xyz(i,3)+10, 300,'white','filled','p','LineWidth',2,'MarkerEdgeColor','k');
            scatter3(xyz(i,1)*xpixel, xyz(i,2)*ypixel, xyz(i,3)+2, 500,'black','filled','p','LineWidth',1.5,'MarkerEdgeColor','white');
        % 端末がエンドデバイス
        elseif (strcmp(nodeList(i).type, 'rt'))
            %scatter3(xyz(i,1), xyz(i,2), xyz(i,3)+10, 300 ,'white','filled','s','LineWidth',2,'MarkerEdgeColor','k'); %[R,G,B]
            scatter3(xyz(i,1)*xpixel, xyz(i,2)*ypixel, xyz(i,3)+2, 300 ,'black','filled','s','LineWidth',2,'MarkerEdgeColor','white'); %[R,G,B]
        % 端末がエンドデバイス
        elseif (strcmp(nodeList(i).type, 'rf'))
            %scatter3(xyz(i,1), xyz(i,2), xyz(i,3)+10, 300 ,'white','filled','^','LineWidth',2,'MarkerEdgeColor','k'); %[R,G,B]
            scatter3(xyz(i,1)*xpixel, xyz(i,2)*ypixel, xyz(i,3)+2, 300 ,'black','filled','^','LineWidth',2,'MarkerEdgeColor','white'); %[R,G,B]
        % 端末がエンドデバイス
        elseif (strcmp(nodeList(i).type, 'cd'))
            %scatter3(xyz(i,1), xyz(i,2), xyz(i,3)+10, 300,'white','filled','o','LineWidth',2,'MarkerEdgeColor','k');
            scatter3(xyz(i,1)*xpixel, xyz(i,2)*ypixel, xyz(i,3)+2, 300,'black','filled','o','LineWidth',2,'MarkerEdgeColor','white');
        end
        % 端末情報の描画
         %text(xyz(i,1)*xpixel+10, xyz(i,2)*ypixel , xyz(i,3) + 7, num2str(nodeList(i).no) ,'Color','black','FontSize',15);
    end
    hold off   %座標軸の開放
    %title('ネットワーク');
end

function PointThreshold = ElectromagneticWaveThreshold(xpixel,ypixel,frequency,gioData,x,y,z,xyse,height,gioMeta)
    [m,n] = size(gioData);
    ...
    for i = xyse(1,1) : xyse(1,2)
        for j = xyse(2,1) : xyse(2,2)
            ...
            PointThreshold(j,i) = ThresholdGain(xpixel,ypixel,frequency,x,y,z,i,j,gioData(j,i)+height(j,i),MaxElevation);
        end
    end
end

function f = ThresholdGain(xpixel,ypixel,frequency,x,y,z,x1,y1,z1,MaxElevation)
    %LoS環境
    ...
    elseif MaxElevation(5) > 0 && MaxElevation(4) <= 0
        f = f - (3*MaxElevation(5));
        %
    end
    %森林浸食
    %
    if MaxElevation(6) >= 0
        f = f - 0.3*MaxElevation(6);
    end
    if MaxElevation(7) >= 0
        f = f - 1.0*MaxElevation(7);
    end
    if MaxElevation(8) >= 0
        f = f - 0.8*MaxElevation(8);
    end
end
...
function MaxElevation = StraightPosition(xpixel,ypixel,frequency,TyuX,TyuY,TyuZ,X,Y,gioData,h,gioMeta)
    ...
    MaxElevation =  [TyuX TyuY TyuZ 0 0 0 0 0];
    Tilt = (TyuY - Y)/(TyuX - X);
    ...
    if X == TyuX  || abs(Tilt) >= 1
        ...
        for i = minY : maxY
            ...
            Stem = (z + 3)*(gioMeta(y,x) == 1000);
            Paddy = (z + 0.6)*(gioMeta(y,x) == 500);
            Forest = Stem + 7;
            ...
            elseif 0 < Forest - TiltZ
                MaxElevation(7) = MaxElevation(7) + 5;
            elseif 0 < Paddy - TiltZ
                MaxElevation(8) = MaxElevation(8) + 5;
            end
        end
...
function [WaveMap,MAPS,maps] = ElectromagneticWaveMap(PointThreshold,threshold,WaveMap,i,xyse,MAPS,maps,gioData)
    PriNum = 2^(i - 1);
    for j = xyse(1,1):xyse(1,2)
        for k = xyse(2,1):xyse(2,2)
            if PointThreshold(k,j) >= threshold
