...
#瀬戸内海全域(この範囲だと全て表示されない)
#slat = 34.398286     #float(slat)
#slon = 132.171420   #float(slon)
#elat = 33.943917     #float(elat)
#elon = 132.723863       #float(elon)
...
fig,ax = plt.subplots()
ax.contourf(XYZ[1,:,:],XYZ[0,:,:],RSSI,alpha=0.6)
ax.plot(XYZ[1,uy,ux],XYZ[0,uy,ux],marker="*",markersize=10,c='red')

xlim = ax.get_xlim()
ylim = ax.get_ylim()
ax.imshow(tile,extent=[*xlim,*ylim], aspect='auto',alpha=0.6)
