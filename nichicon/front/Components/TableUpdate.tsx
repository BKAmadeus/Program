                            ...
                            {Object.keys(TC.data.table[TC.index]).map((key:any,index:number)=>{
                                if(key in TC.data.association){
                                    ...
                                    return(
                                        <TableCell key={index}>
                                            {TC.data.table[TC.index][key]}
                                            {['id','code'].includes(key)?"":<TextField defaultValue={TC.data.table[TC.index][key]} onChange={(e)=>{TC.data.table[TC.index][key] = e.target.value}}/>}
                                        </TableCell>
                                    )
                                }
                                return ""
                                        ...
                                        <TableBody>
                                            {TC.data.table[TC.index][key].map((val:any,index2:number)=>(
                                                <TableRow key={index2}>
                                                    ...
                                                    {TC.data.struct[key].map((key2:any,index3:number)=>(
