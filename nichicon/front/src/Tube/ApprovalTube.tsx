...
import { col,colorList,TubeIndex,SokoIndex,VerificationList,ApprovalList,ApprovalList2,AllAuthorityList } from "../Data/Data"
            ...
            <CT>
              ...
              <div className='list'>
                <List sx={{ width: '100%', maxWidth: 360, bgcolor: 'background.paper', position: 'relative', overflow: 'auto', maxHeight: 500, '& ul': { padding: 0 } }} subheader={<li />}>
                  {tables.display.table?tables.display.table.filter((val:any)=>parseInt(val.available) === Math.ceil(state)).map((val:any,index:number)=>{
                    return(
                      <li key={index}>
                        <ul>
