...
export function TableAdd(TC:any) {
    ...
    const handleSubmit = () => {
        TC.setValue(`reEffect`,!TC.watch(`reEffect`));
        func.postData({flg:"TableUpdate",data:TC.data.table[0],table_name:TC.TableName,struct:TC.data.struct,association:TC.data.association,UserData:UserData})
        .then((data:any)=>{
            if(data === 'error'){
                setFlg(true);
            }
    ...
    const handleAddStruct = (key:string) =>{
        ...
        if(TC.data.table[0][key] === null){
            TC.data.table[0][key] = [];
            TC.data.table[0][key].push(ans);
        }
        else{
            TC.data.table[0][key].push(ans);
        }
    ...
    const handleRemoveStruct = (key:string,index:number) =>{
        console.log(TC.data.table[0][key],index);
        TC.data.table[0][key].splice(index,1);
        reEffect(!effect);
    }
        ...
        <Dialog fullScreen open={open} onClose={handleClose}>
            ...
            <TableContainer component={Paper}>
                <Table>
                    <TableHead>
                        <TableRow>
                            {Object.keys(TC.data.table[0]).map((key:any,index:number)=>{
                        ...
                        <TableRow>
                            {Object.keys(TC.data.table[0]).map((key:any,index:number)=>{
                                if(key in TC.data.association){
                                    return (
                                        <React.Fragment>
                                            {Object.keys(TC.data.table[0][key]).map((key2:any,index2:number)=>{
                                                return (
                                                    <TableCell key={key+index2}>
                                                        {TC.data.table[0][key][key2]}
                                                    </TableCell>
                                                )
                                            })}
                                        </React.Fragment>
                                    ...
                                    return(
                                        <TableCell key={index}>
                                            {key === 'id'?"入力不可":
                                            <TextField onChange={(e)=>{TC.data.table[0][key] = e.target.value}}/>
                                            }
                                        </TableCell>
                                    )
                                }
            ...
            <DialogTitle>配列データ</DialogTitle>
                {Object.keys(TC.data.table[0]).map((key:any,index)=>{
                            ...
                            return (
                                <TableContainer component={Paper} key={"struct"+index}>
                                    <Table>
                                        ...
                                        {TC.data.table[0][key]?
                                        <TableBody>
                                            {TC.data.table[0][key].map((val:any,index2:number)=>(
                                                <TableRow key={index2}>
                                                    <TableCell></TableCell>
                                                    ...
                                                    {Object.keys(TC.data.struct[key]).map((key2:any,index3:number)=>(
                                                            <TableCell key={index3}>
                                                                <TextField defaultValue={val[key2]} onChange={(e)=>{TC.data.table[0][key][index2][key2] = e.target.value}}/>
                                                            </TableCell>
                                                    ))}
                                                </TableRow>
                                            ))}
