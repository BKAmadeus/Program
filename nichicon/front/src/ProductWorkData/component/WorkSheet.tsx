...
import {CapacitanceToleranceLevelInside,MachiningRideArray,MachiningTapingArray,MachiningLFormingArray} from '../../Data/Data';
import type { FormProps } from '../../Data/Data';
import { useForm } from 'react-hook-form';
import {Base} from './01Base';
import {Winding} from './02Winding';
...
export function Sheet(FSD:any) {
    ...
    React.useEffect(()=>{
        var post = {flg:"WorkDataCalculation",Product:FSD.val,tables:FSD.tables};
        func.postData(post)
        .then((data:any) => {
            console.log("chack:",{data:FSD.val,cul:data.culculation,WorkData:data.WorkData});
            setData({data:FSD.val,cul:data.culculation,WorkData:data.WorkData});
            DataSetConstant(FSD.val,setValue,true,true);
            DataSetCalculation(data.culculation,setValue,watch);
        })
        // eslint-disable-next-line react-hooks/exhaustive-deps
    },[])
    ...
    return (
        ...
        {Data? 
        <div>
            <Base Data={Data} register={register} watch={watch} setValue={setValue}/>
            <Winding Data={Data} register={register} resetField={resetField} watch={watch} setValue={setValue} tables={FSD.tables}/>
